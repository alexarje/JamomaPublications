...
% TODO: Possible re-naming:
%   Jamoma Audio Graph		[Tim votes for this one]
%   Jamoma DSP Graph
...
\begin{figure*}[!t]
\centering
\subfigure[Without Multicore, using MSP audio signals for 3 inputs and 16 loudspeakers] 
...
% TODO: Finish of this section by referencing \ref{fig:max-before-after}, how it illustrates MSP to Multicore bridging, matrix, FX and output using PortAudio. Also comment on how much simpler it is to reconfigure the number of speakers: We now only need to change attribute values in jcom.matrix≈ and jcom.dbap, instead of having to do repatching. (Save latter for discussion)
...
% TODO: Discuss, either here or later on, the future potential for prototyping in Max, and turn into C++ code for e.g. VST and AudioUnit plugins.
...
% TODO: is it efficiency, or does the word 'cost' apply to the above?  [TAP]
...
%		we believe we have created a context in which code is ‚Äö√Ñ√∂ pleas-
%		ant to work with."
%
...
% TODO - assigned to Tim and Nils
Commonly todays computer systems use parallelized CPU architectures. UC Berkeley's Parallel Computing Laboratory identified audio applications  as one of the most promising but also challenging use cases of parallel computing \cite{asanovic2008parallel}. 
Multichannel real-time audio computing requires low latency and jitter-free audio processes. Therefore, race condition and deadlocks need to be prevented.

The Jamoma Mutlicore Audio Graph Layer already allows to run separate graphs on separate processing threads. Although one graph has not been used within a multi-threading scenarios yet, we do see its potential for it.

Through the implemented pulling strategy (Section \ref{sec:pull}), the audio graph structure is already being analyzed and benchmarked at the sink. As a next step, according to its graph structure, the involved audio processes could then be distributed and managed amongst different CPUs. A promising management strategy can be found in \cite{PartzschAES122}.
...
% TODO: I thought this was true, but the 'addressing different hardware' part seems to not be true at the moment -- all jcom.dac≈ object's are sharing a single point of access to hardware internally.  Maybe this is a feature request though?  [TAP]
...
% TODO: we need an object like out‚Äö√Ñ√∂‚àö¬¢‚àö‚Ä† that drives a graph outside of real time to fill a buffer~ or jit.matrix

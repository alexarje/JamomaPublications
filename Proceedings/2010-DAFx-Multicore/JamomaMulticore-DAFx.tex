...
Many frameworks, toolkits, and environments for real-time audio processing fuse the issues of creating unit generators with creating graph structures that process audio through those unit generators.  Within the Jamoma Platform there is a clear separation of concerns, structured according to a layered architecture of frameworks providing a comprehensive infrastructure for creating computer music systems \cite{Place:2010}. Five frameworks currently comprise the Jamoma Platform: Jamoma Foundation, Jamoma DSP, Jamoma Multicore, Jamoma Graphics and Jamoma Modular (see Figure~\ref{fig:layers}). Jamoma Foundation provides low-level support, base classes, and communication systems; Jamoma DSP specializes the Foundation classes to provide a framework for creating a library of unit generators; Jamoma Graphics provides screen graphics and Jamoma Modular provides a structured approach to development and control of modules in the graphical media environment Max \cite{Place:2006}. 

Jamoma Multicore provides a method by which Jamoma DSP objects may be networked together in real-time. 
...
n the Jamoma Platform there is a clear separation of concerns: Jamoma Foundation provides low-level support, base classes, and communication systems; Jamoma DSP specializes the Foundation classes to provide a framework for creating a library of unit generators; Jamoma Multicore provides a method by which Jamoma DSP objects may be networked together in real-time. 

(comparing to CLAM):

By their nature, all classes inheriting from the Jamoma Foundation's base class, TTObject, support dynamically-bound asynchronous message passing (though not in a graph structure).  Jamoma Multicore, then, only needs to create a mechanism for synchronous graph calls.
...
%		we believe we have created a context in which code is â pleas-
%		ant to work with."
%
...
% Who has a true multichannel environment?  anyone?
% ZenGarden, importing the Pd patcher file format to create a graph (anything more to say that we didn't say in the DSP paper?) 
% TODO: Discussing Max and Pd makes sense -- changing the number of channels is changing the code
% TODO: SuperCollider - mention that it is possible to work on arrays of signals in SuperCollider, while this is not easy to do in Max?
...
% Some interesting ones: Max4Live, Flext (for Pd and Max) -- the point is that flext is slightly more generic than just Max
...
Domain-Specific Text-Based Languages are similar to the Max Family except that they create networks of Unit Generators using a text interface.  I don't really want to talk about CSound.  Is that really necessary?  Can I just mention CSound and SuperCollider together in one sentence?
...
Second, the audio is pulled from the object above it in the chain by calling a 'process' method, again initiated by the terminal object.
...
% TODO: Tim fix jcom.unpackÅ object in subpatchers bug
...
One thing is Live Coding (cite OrgSound article) using irb.  That article has some great ideas and general discussion about Live Coding.  One topic is the use of general purpose languages such as Ruby vs. domain specific languages such as ChucK.  With our extensions of the Ruby environment we hope to bridge the best of both worlds.
...
% TODO: we need an object like outâ‰ˆ that drives a graph outside of real time to fill a buffer~ or jit.matrix
...
A potential solution to this problem (as stated by Gino Robair): "Because in 15 years, I will still be using my hardware synths and effects, but I can guarantee that every bit of software I'm using todayâ along with its sessions, patches, and stored settingsâ will be inaccessible, either through lack of developer support, because the host computer has died, or because subsequent upgrades no longer open the files I created with today's version."

...
%According to the outlined main applications and the functionalities provided by the co-existing Jamoma frameworks (Figure \ref{fig:layers}), the Jamoma Multicore Audio Graph Layer has the following requirements: 
...
The Jamoma frameworks align with contemporary philosophies for good coding practice, facilitating the readability, debugging, maintenance, and distribution of code.
...
A general system for developing methods for testing audio signal processes and graphs remains to be developed, but is likely to be much simpler to achieve using the text-based Jamoma Multicore implementation for Ruby than it would be within the graphical patching environments Max or Pd.
This is a long term investment in keeping the code base maintainable, well-documented, stable and extendable, hopefully also encouraging others to make use of the frameworks and join in contributing to future development.

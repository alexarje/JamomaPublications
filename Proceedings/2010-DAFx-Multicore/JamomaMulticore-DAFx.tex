...
The Jamoma Audio Graph (JAG) layer is a framework for creating graph structures in which unit generators are connected together to process multichannel audio in real-time.  
This layer forms part of the Jamoma layered architecture for interactive systems.
Jamoma Audio Graph is particularly well-suited to spatial audio contexts demanding large numbers of audio channels, such as Higher Order Ambisonics, Wave Field Synthesis and microphone arrays for beamforming.
% TODO: I'm not convinced that we currently should mention WFS, as we don't have support for it anywhere in Jamoma so far.
Current implementations of Jamoma Audio Graph target the Max/MSP, PureData, Ruby, and AudioUnit environments.
...
Many frameworks, toolkits, and environments for real-time audio processing fuse the issues of creating unit generators with creating graph structures\footnote{Wikipedia defines this by saying ``a graph is an abstract representation of a set of objects where some pairs of the objects are connected by links''. \url{http://en.wikipedia.org/wiki/Graph_(mathematics)}} that process audio through those unit generators.
...
Alternatively, the Jamoma Platform implements a clear separation of concerns, structured in a layered architecture of several frameworks \cite{Place:2010}.
 ...
 These frameworks are: Jamoma Foundation, Jamoma Graphics, Jamoma Modular, Jamoma DSP, and Jamoma Audio Graph (see Figure~\ref{fig:layers}).
% TODO: Add Jamoma Graph to this discussion. 
% TODO: Jamoma Graph should be renamed as well, e.g. to Jamoma Async Graph.
% TODO: update the figure to include Jamoma Graph and possibly make the look-and-feel more consistent with other figures in the paper? All the different background colors look bewildering, don't add anything to the general understanding of the concepts (each framework is already demarked by the boundaries), and look bad in print
 ...
 Jamoma Audio Graph, the focus of this paper, is an open source C++ framework providing the ability to create and network Jamoma DSP objects into dynamic graph structures for audio processing\footnote{Licensing for all layers of the Jamoma Platform are provided under the terms of the GNU LGPL}.  
...
%According to the outlined main applications and the functionalities provided by the co-existing Jamoma frameworks (Figure \ref{fig:layers}), the Jamoma Audio Graph Audio Graph Layer has the following requirements: 
...
% TODO: The above paragraph is nice, but I still feel that we jump into the following bulleted list in a way that make the flow of the text feel awkward here. - TL
\begin{packed_item}
...
% In addition to the decoupling of the unit generators (Jamoma DSP) from the processing graph, Jamoma Audio Graph differs from other audio graph architectures in several important ways. %NP: do we need this prev. sentence? isn't that clear ? 
...
This allows for Jamoma Audio Graph to be readily implemented for any number of environments.  
...
% TODO: Would it be appropriate to mention OSC and control data in general (from GUI or external interfaces) in passing here? - TL
...
The architecture of \texttt{TTAudioGraphObject} is layered: the audio graph object itself only knows about its inlets; the inlets only know about their sources (joins); the sources know about what object and from which outlet they are connected. 
...
% TODO: In section 1.1 I)mplementation we state that we have implemented JAG for Audio Unit. Don't Audio Unit then need to be added to this section? - TL
...
% Currently a multichannel signal is considered to be a complex signal with channel 1 being the real part and channel 2 being the imaginary part -- the design here has never really be discussed yet by us...  [TAP]

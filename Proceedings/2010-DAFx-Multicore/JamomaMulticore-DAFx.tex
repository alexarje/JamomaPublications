...
Many frameworks, toolkits, and environments for real-time audio processing fuse the issues of creating unit generators with creating graph structures that process audio through those unit generators.  Alternatively, the Jamoma Platform implements a clear separation of concerns, structured in a layered architecture of several frameworks. Five frameworks currently comprise the Jamoma Platform, providing a comprehensive infrastructure for creating computer music systems. These frameworks are: Jamoma Foundation, Jamoma DSP, Jamoma Multicore, Jamoma Graphics, and Jamoma Modular (Figure~\ref{fig:layers}). Jamoma Foundation provides low-level support, base classes, and communication systems; Jamoma DSP \cite{Place:2010} specializes the Foundation classes to provide a framework for creating a library of unit generators. Jamoma Multicore, the focus of in this paper, is an open source C++ framework to create and network Jamoma DSP objects into dynamic graph structures for audio processing\footnote{Licensing is provided under the terms of the GNU LGPL}. 
Jamoma Graphics is used for creating cross-platform graphical user interfaces. An finally, Jamoma Modular, a structured approach to development and control of modules in the real-time media environment Max/MSP \cite{Place:2006}.  
...
% CHANGED: moved to the end of the Csound section instead of after the SuperCollider section -- it was looking like it was all one example [tap]

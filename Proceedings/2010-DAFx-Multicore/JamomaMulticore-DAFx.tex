...
The Jamoma Audio Graph Layer (JAGL) is a framework for creating graph structures\footnote{Wikipedia defines this by saying ``a graph is an abstract representation of a set of objects where some pairs of the objects are connected by links''. \url{http://en.wikipedia.org/wiki/Graph_(mathematics)}} in which unit generators are connected together to process multichannel audio in real-time.  
The Jamoma Audio Graph Layer is currently implemented in Max/MSP, PureData, Ruby, and AudioUnits.
The Jamoma Audio Graph is particularly well-suited to spatial audio contexts demanding large numbers of audio channels, such as Higher Order Ambisonics, Wave Field Synthesis and microphone arrays for beamforming.
This layer forms part of the Jamoma layered architecture.

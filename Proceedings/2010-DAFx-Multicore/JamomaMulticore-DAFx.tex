...
Compared to the above SuperCollider has an elegant way of representing multiple channels of audio as arrays. When an array is given as input to a unit generator it causes multichannel expansion: An array of multiple copies of that unit generator is made, each with a different value from the input array. In the following example a stereo signal containing white and pink noise respectively is filtered:

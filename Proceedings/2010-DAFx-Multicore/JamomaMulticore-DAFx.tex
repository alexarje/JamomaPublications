...
In Csound multichannel audio graphing is extended somewhat through the introduction of the Zak Patch System and the "chn" set of opcodes. Zak enables flexible routing from one instrument to another as instruments may be reconfigured in the score without requiring changes to the orchestra \cite{Mikelson:2000} and is used e.g. by the \texttt{vbapz} opcode for multichannel vector-base amplitude panning. The "chn" opcodes allow for accessing a global string-indexed software bus enabling communicating between a host application and the Csound engine, but can also be used for dynamic routing within Csound itself \cite{Yi:2006}. Below is an example of  a simple multichannel filter implementation using named software busses to iterate through the channels:

% Note: This example was provided by �yvind Brandsegg. The code have proper spacing when typeset.
% TODO: I'm currently checking with �yvind to ensure that the example manage to take care of xm1, xm2, ym1 and ym2 for each channel from one vector to the next. [TL]
...
SuperCollider has an elegant way of representing multiple channels of audio as arrays. When an array of audio signals is given as input to a unit generator it causes multichannel expansion: multiple copies of the unit generator create an array of unit generators, each processing a different signal from the array of inputs. In the following example a stereo signal containing white and pink noise is filtered:
...
{\framebox{\includegraphics[height=0.24\textheight]{jalg-sur-vbap-before.png}}} \hspace{0.7cm}
\subfigure[With Multicore]
{\framebox{\includegraphics[height=0.24\textheight]{jalg-sur-vbap-after.png}}}
\caption{The Jamoma signal processing Max patch for Vector Based Amplitude Panning (VBAP)}
...
Figure \ref{fig:max-before-after}(a) shows the signal processing patch of the Jamoma module for spatializing incomming sounds using Vector Base Amplitude Panning (VBAP) \cite{Pulkki:1997vbap}. According to the user-defined number of incoming (to be spatialized) sounds and number of loudspeakers, the internal mixing matrix and its connections were dynamically scripted which can create certain stress on Max for time critical situations. Furthermore a change in the DSP structure requires a rebuild of the DSP chain.   

Jamoma Multicore externals are currently subsequently introduced into Jamoma Modular to simplify and the spatialization modules and make them more robust because scripting can be entirely avoided. Figure \ref{fig:max-before-after}(b) shows a new version of the patch from Figure \ref{fig:max-before-after}(a), employing the new Jamoma Multicore Audio Graph. The new object jcom.matrix$\approx$ performs mixing operations between the audio streams of the incoming multicable and outputs the audio feeds for each loudspeaker. 
...
Jamoma Multicore was also implemented for Pure Data which is essentially similar to Jamoma Multicore for Max/MSP. The most obvious difference between Multicore for Pure Data and Max may be the slightly different external names, having '=' instead of '$\approx$' appended as an indicator for multichannel signal streams. Figure \ref{fig:pd} demonstrates the use of Jamoma Multicore in PD for passing 4 channels through a chain for processing, taking advantage of the multichannel patch cord connections.  

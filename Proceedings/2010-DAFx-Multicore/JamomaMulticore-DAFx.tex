...
% especially with a massive nubmer of audio channels (Higher Order Ambisonics, WFS, microphone arrays for beamforming)
\end{abstract}
...
When visualizing a graph for audio processing two-dimensionally it is common to represent the flow of audio from top to bottom or left to right.  Under the hood, the processing is often implemented in this top-to-bottom flow as well.  Audio at the top of the graph `pushes' down through each subsequent object in the chain until it reaches the bottom.
...
In the figure one object is connected upstream to three objects, and downstream to two objects, with all connections being multichannel. Two objects are connected to the leftmost inlet, two to the second, one to the third and so on. 

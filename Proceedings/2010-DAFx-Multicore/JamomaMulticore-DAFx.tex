...
The previous scripting and patching hack approach is currently being replaced by JAG objects to simplify the spatialization modules and make them more robust.
Figure \ref{fig:max-before-after}b indicates how the number of sources and speakers can be changed simply by updating attribute values for a few JAG objects, thus eliminating the need for patcher scripting in Jamoma Modules.
...
Figure \ref{fig:pd} demonstrates the use of JAG in Pd for passing 4 channels through a chain for processing, taking advantage of the multi-channel patch cord connections.  

\begin{figure}[htbp]
\centerline{\framebox{
	\includegraphics[width=0.7\columnwidth]{multicore-pd}}}
\caption{Pd patch, processing 4 channels with JAG}
\label{fig:pd}
\end{figure}
...
Additional plans include the introduction of upmix/downmix algorithms into the Audio Graph to seamlessly mediate between different multi-channel media formats including compressed spatial audio material (e.g. mp3-surround, DirAC). 
Here, we are planning to explore the potential to attach additional meta-information to the individual audio channels to ensure that multichannel audio material is correctly processed (e.g. to identify the signal components in an Ambisonics B-format, or to distinguish a binaurally processed audio stream from a conventional stereo track). 

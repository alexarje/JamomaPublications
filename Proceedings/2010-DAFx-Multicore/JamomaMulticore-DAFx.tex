...
Alternatively, the Jamoma Platform implements a clear separation of concerns, structured in a layered architecture of several frameworks \cite{Place:2010}.
 ...
 Jamoma Multicore, the focus of this paper, is an open source C++ framework providing the ability to create and network Jamoma DSP objects into dynamic graph structures for audio processing\footnote{Licensing is provided under the terms of the GNU LGPL}.  
...
Environments for processing typically need to address concerns in both synchronous and asynchronous contexts.  
...
These blocks of samples are passed using the Jamoma DSP class for audio signals, which bundles vectors of audio for multiple channels together with an interface for accessing the signal data and metadata.
...
This `pull' method for processing and audio graph is used by several environment's including Apple's AUGraph and ChucK \cite{wang:2008}. 
...
In many for real-time audio patching environments, such as Max/MSP, PD, Bidule or AudioMulch, audio objects are connected using mono signals. 
...
Max, Pd, SuperCollider, Csound, Bidule, AudioMulch, Reaktor, Reason all have have units, but the units can only be used within the particular environment. 
The units have a proprietary format for unit generators, so there's no interchangeability, and likewise the graph structure is proprietary. 
While SDKs are available for some of them, for others no SDK exists at all and they are proprietary. 
Ability to host Audio Units and VST extends these environments, but with limitations.
...
A connection between objects can be considered to be a many-to-many association; each outlet can be connected to many inlets, and each inlet can be connected to many outlets. 
...
For processing to occur, the connections of the graph must be established and the graph readied for operation.  
This is done by first passing a source object and the outlet and inlet number of the connection to the downstream object's \texttt{connect()} method.  
...
When asked for a vector of audio by the unit generator, they then each request audio from all of their sources (other objects' outlets).  
...
The multichannel signals are represented using ordinary patch chords in the Max environment. 
Interfacing between MSP and Multicore is facilitated by the externals jcom.pack$\approx$ and jcom.unpack$\approx$, packing and unpacking multiple MSP audio signals onto and off a multichannel signal. 
...
A number of multi-channel audio generators for noise and common waveforms are available.
...
% DONE - interfacing with MSP: jcom.pack$\approx$ and jcom.unpack$\approx$
% DONE - splice and slice: jcom.join$\approx$, jcom.split$\approx$
% DONE - audio in/out using PortAudio: jcom.adc$\approx$  and jcom.dac$\approx$ (do we need reference for PortAudio?)
% DONE - audio generators: jcom.noise$\approx$, jcom.phasor$\approx$, jcom.wavetable$\approx$
% - multichannel audioFX: jcom.op$\approx$, jcom.dcblocker$\approx$, jcom.degrade$\approx$, jcom.delay$\approx$, jcom.filter$\approx$, jcom.limiter$\approx$, jcom.overdrive$\approx$, jcom.pulsesub$\approx$
% - spectral: jcom.fft$\approx$, jcom.window$\approx$
...
According to the user-defined number of incoming (to be spatialized) sounds and number of loudspeakers, the internal mixing matrix and its connections were dynamically scripted which can create certain stress on Max for time critical situations. 
Furthermore a change in the DSP structure requires a rebuild of the DSP chain.   

Jamoma Multicore externals are currently subsequently introduced into Jamoma Modular to simplify and the spatialization modules and make them more robust because scripting can be entirely avoided. 

...
% TODO: alternative title: The Jamoma DSP Platform : A Flexible and Dynamic Framework for Audio Signal Processing in C++
% 
...
\end{abstract}
% TODO: is it a framework or an API, as stated in the first sentence of the abstract ?
...
% TODO: come back and look this next paragraph
...
%Additional tools and facilities have been fabricated to make Jamoma DSP's unit generators readily available in several popular environments.
...
%Given a Jamoma DSP unit generator, this object can be easily compiled for Cycling '74's Max/MSP environment.
...
This object dynamically loads any of the currently  22 filter classes from the DSP Library. The classes are loaded and swapped on-the-fly, without requiring Max's DSP chain to be rebuilt.  The code for the \emph{\small{jcom.filter$\sim$}} external searches the object registry for available filter classes to present a list of choices for the user. This list will then be dynamically updated if new filters are added without requiring the external to be recompiled.   
...
% implications of the library, what potentials does it have



% potoential for rapid prototyping - and DRY stuff
% user-extensible (at runtime)
% queryable -- potential for mappings
% boring stuff taken care of -- clean code
% license works for us
...
% potential for rapid prototyping - and DRY stuff - % boring stuff taken care of -- clean code
% expand wrappers to more environments
% Add common audio processing stuff (spectral, granulation, +++) - non-linear filters would be cool - add support for additional spatialisation algoritms such as VBAP, ambisonics, DBAP\cite{Lossius:2009} to support ongoing development within Jamoma Modular (refs to Porto and DBAP)
% Scheduler - do we need to include this in the discussion?
% user-extensible (at runtime) -- so it can be expanded even more



% queryable -- potential for mappings
% Exploring implicit patching paradigms - also for dynamic rerouting between modules \cite{Bray:2005}


% license works for us


% benchmark and optimize --- out?




The class wrapper demonstrated in Section \ref{sec:classwrapper} illustrates part of the power of the Jamoma Foundation for use as a \emph{rapid prototyping environment}.  It leverages all of the features of dynamic binding to make a class available to many different environments on many platforms with a minimal amount of coding and effort required.  The code base is further strengthened because this architecture provides a practical application of the DRY principle for representing concepts in a single authoritative location.  We can combine objects together in the environment of our choice: Max, Pd, audio sequencer (using plug-ins), or a web-browser using Ruby on Rails.  Then we can move the code from one environment to another easily, or port it back to C++ with minimal effort.

%   TODO: If space, this is the place to discuss this: combining unit generators is simple - example: combining simple units into compound (e.g. DC Blocker)
...
% Add common audio processing stuff (spectral, granulation, +++) - non-linear filters would be cool - add support for additional spatialisation algoritms such as VBAP, ambisonics, DBAP\cite{Lossius:2009} to support ongoing development within Jamoma Modular (refs to Porto and DBAP)
% Scheduler - do we need to include this in the discussion?
%Exploring implicit patching paradigms


The class wrappers can be expanded to cover more environments, such as VST, SuperCollider, and CSound.
...
% TODO: Reconnecting things on the fly can be expanded into: 
...
% TODO: Form as a proper sentence.

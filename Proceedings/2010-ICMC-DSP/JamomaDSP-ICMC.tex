...
% TODO: I think the following should be use bullet items as the list below.  Without them it looks unclear to me in the PDF. [tap] Trond agrees, currently it looks messy. If it is not to be a bulledet list at least the sentences will need to end with a dot.
...
% TODO: IMHO the footnote on whether Maxing is programming or not is not very interesting or relevant here, and could be left out. Similarly the Apple corporate control of Objective-C is not very interesting. If this principle really was important to us, we would look upon Max with a similar sceptisism, being owned by C'74.  [TL]
...
Jamoma, originially conceived as a modular standard for structuring patchers in Max \cite{Place:2006}, has now evolved into a layered architecture of frameworks that provide essential %TODO: I think it's more than 'esential', should be 'extensive' or so [np]  
 infrastructure for creating computer music systems in general, not just for Max/MSP. This section presents each of the five frameworks which are entitled Jamoma Foundation, Jamoma DSP, Jamoma Multicore, Jamoma Graphics and Jamoma Modular (see Figure~\ref{fig:layers}).
We will emphasis on the Jamoma Foundation and DSP layer to show how the requirements outlined in Section \ref{sec:Requirements} are achieved.    
...
%The Jamoma Platform currently consists of five frameworks: Jamoma Foundation, Jamoma DSP, Jamoma Multicore, Jamoma Graphics and Jamoma Modular. This paper is focusing on the Jamoma Foundation and Jamoma DSP frameworks. 
%Figure~\ref{fig:layers} illustrates the layers of the Jamoma Platform.  
%Each colored shape is implemented as it's own framework. 
